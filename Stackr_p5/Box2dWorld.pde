// adapted from Daniel Shiffman's Box2dProcessing
...
public class Box2DWorld {
  ...
  public Box2DWorld( float scale, Vec2 center)
  {
    transX = center.x;
    transY = center.y;
    yFlip = -1;
  ...
  public Vec2 coordWorldToPixels(float worldX, float worldY) {
    ...
    if (yFlip == -1.0f) pixelY = map(pixelY,0f,height, height,0f);
  ...
  public Vec2 coordPixelsToWorld(float pixelX, float pixelY) {
    ...
    float worldY = pixelY;
    if (yFlip == -1.0f) worldY = map(pixelY,height,0f,0f,height);
